...
uint16_t ADC_read_blocking(void) {
    ...
    while (((((LPC_ADC_TypeDef *) ((0x40000000UL) + 0x34000) )->ADGDR) & (1UL << 31)) == 0) {

    }

    result = (((LPC_ADC_TypeDef *) ((0x40000000UL) + 0x34000) )->ADGDR >> 4) & 0xFFF;
    return (uint16_t)result;
}
...
uint16_t ADC_get_millivolts(void) {
    return (uint16_t)(((uint32_t)AD_current * 3300U) / 4095U);
}


float ADC_get_voltage(void) {
    return ((float)AD_current * ((float)3300U / 1000.0f)) / (float)4095U;
}


uint8_t ADC_get_percent(void) {
    return (uint8_t)(((uint32_t)AD_current * 100U) / 4095U);
}


uint32_t ADC_map(uint32_t out_min, uint32_t out_max) {
    if (out_max >= out_min) {
        return out_min + ((uint32_t)AD_current * (out_max - out_min)) / 4095U;
    }

    return out_min - ((uint32_t)AD_current * (out_min - out_max)) / 4095U;
}
